...
data.frame(anc=c("1","2"),
  ...
  rm(anc_donor,
    ...
    MARKER <- "/DATA/home/mjahani/LINKADE_DRAG/GP/Annuus.ann_fullsam.tranche90_snps_bi_AN50_beagle_AF99.maf0.03_286sample_rrBLUP.in"
    PHENOTYPE <- "/DATA/home/mjahani/LINKADE_DRAG/GP/phenotype_common_georgia_corrected.csv"
    SAMPLES <-  "/DATA/home/mjahani/LINKADE_DRAG/GP/Annuus.ann_fullsam.tranche90_snps_bi_AN50_beagle_AF99.maf0.03_286sample.sample_list"
    SNPs <-  "/DATA/home/mjahani/LINKADE_DRAG/GP/Annuus.ann_fullsam.tranche90_snps_bi_AN50_beagle_AF99.maf0.03.SNP_LIST"
...
bedtools genomecov -i introgression_annuus.bed -g HA412v2_chromosome.bed -bga | awk '{print $1"\t"$2"\t"$3"\t"$4/574 }'  >  #calculate the base frequency
...
awk '{ if ($4 > 0) {print $1"\t"$2"\t"$3"\t"$4} }' introgression_annuus.coverage.shiny | bedtools merge | wc -l

awk '{ if ($1 == 2) { print $5,"\t",$2,"\t",$3} }' pcadmix.xxx2.regions.txt | sed 's/ //g' | bedtools sort -i > introgression_secondarygemplasm.bed
bedtools genomecov -i introgression_secondarygemplasm.bed -g HA412v2_chromosome.bed -bga  > introgression_secondarygemplasm.coverage
