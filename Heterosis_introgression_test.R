...
for (trait in 1:length(TRAIT_ID)) {
  ...
  BLUPS %>%
    ...
    mutate(P= (t.test(pull(filter(BLUPS,SMPLES == "full_SAM",TRAIT == TRAIT_ID[trait]),BLUP),
  ...
  BLUPS %>%
    ...
    mutate(P = (prop.test(x = c(as.numeric(full_SAM_pos_frq),as.numeric(highly_homozygous234_pos_frq)),
...
data.frame(TRAIT = c("height","head_weight", #Strong heterosis
                     ...
                     "stigma_antho","oil","leaf_sla","dtf","disk_antho","branching" ) ,#No heterosis 
           Heterosis = c("Strong_heterosis","Strong_heterosis",
