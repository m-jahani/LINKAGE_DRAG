...
#introgressions for each donor, sample and haplotype
  INTROG_SAMPLE %>% 
    filter(sample == as.character(SAMPLES[SAMPL]),
 ...
 #find overlap between 1kb windows and introgressions of each donor, sample and haplotype
  system(paste0("bedtools intersect -wa -a ",
                SAVE_DIR,
 ...
 #remove introgression of each loop 
  ...
  fread(paste0(SAVE_DIR ,
    ...
    distinct(ID) %>%
    mutate(!!paste0(SAMPLES[SAMPL],"_",HAPLOTYPES[HAPLO]) := 2) %>% #!!paste0(SAMPLES[SAMPL],"_",HAPLOTYPES[HAPLO]) :
...
RESULT %>%
  ...
  enframe() %>%
  select(value) %>%
  separate(value, into = c("sample","haplotype"),
           sep = "_",
           remove = F) %>%
...
select(sex, )
 ...
 INTROG_SAMPLE %>% 
    filter(sample == as.character(DONOR_SAMPLE_HAP[i,1]),
           haplotype == as.character(DONOR_SAMPLE_HAP[i,2]),
           donor == as.character(DONOR_SAMPLE_HAP[i,3])) %>%
    mutate(chr_num = as.numeric(gsub("Ha412HOChr","",CHR))) %>% 
    arrange(as.numeric(chr_num),
            as.numeric(start)) %>% 
    select(CHR,
      ...
      fwrite(paste0(SAVE_DIR,
                    "/",
                    DONOR_SAMPLE_HAP[i,1],
                    "_HAP",
                    DONOR_SAMPLE_HAP[i,2],
                    "_",
                    DONOR_SAMPLE_HAP[i,3],
                    ".introgression.bed"),
             sep = "\t",
             col.names = F,
             quote = F)
  ...
  fread(paste0(SAVE_DIR ,
    ...
    mutate(ID = paste0(V1,":",V2,"-",V3)) %>%
    distinct(ID) %>%
    mutate(!!paste0(DONOR_SAMPLE_HAP[i,1],"_",DONOR_SAMPLE_HAP[i,2]) := 1) %>% #!!paste0(DONOR_SAMPLE_HAP[i,1],"_",DONOR_SAMPLE_HAP[i,1]) :
    full_join(.,RANGES) -> RANGES
