...
for (trait in 1:length(TRAIT_ID)) {
  ...
  as.data.frame(
    as.matrix(
      mixed.solve(phenotype[,trait], #phenotyoic data
                  ...
                  method="REML",
                  SE=FALSE, 
                  return.Hinv=FALSE)$u)) -> BLUPs #calculation of BLUPs
  BLUPs %>%
    summarise(!!TRAIT_ID[trait] := mean(V1)) %>%
    mutate(data_type = "observation") %>%
    select(data_type, everything()) %>%
    fwrite(paste0(SAVE_DIR,"/Average_PERMUT_result_",TRAIT_ID[trait]),
           sep = "\t",
           col.names = F,
           quote = F,
           append = T)
  ...
  BLUPs %>% 
    ...
    fwrite(paste0(SAVE_DIR,"/postive_prop_PERMUT_result_",TRAIT_ID[trait]),
           sep = "\t",
           col.names = F,
           quote = F,
           append = T)
...
fread(paste0(SAVE_DIR,"/Average_PERMUT_result_",TRAIT_ID[trait]),header=F) %>%
...
fread(paste0(SAVE_DIR,"/postive_prop_PERMUT_result_",TRAIT_ID[trait]),header=F) %>%
  filter(V1 != "observation") %>%
       ...
       summarise(P = n()/as.numeric(nrow(AVERAGE_PERMUT_result))) %>% 
         ...
         rbind(.,
               filter(AVERAGE_PERMUT_result,V2 > as.numeric(select(AVERAGE_trait_result,V2))) %>%
                 ...
                 mutate(Significant=ifelse(P<0.05,"Significant","non_Significant")) %>%
                 select(TRAIT,P_type,P,Significant)) %>%
         fwrite(paste0(SAVE_DIR,"/P_VALUES_AVERAGE"),
                sep = "\t",
                col.names = F,
                quote = F,
                append = T)
